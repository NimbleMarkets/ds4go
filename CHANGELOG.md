 ...
 * **Fixes**: a double free after a failed multi-image append, `/read` history tied to the file path, a zero-entry encoder cache that still cached, `Session.TokenLogprob` failing on every real call, and empty completions from the example server; the example server parses completions under the request's `reasoning_effort` and the engine's tool syntax; prompt replay asks the engine (`Engine.ThinkModeEnabled`) rather than the default library whether reasoning is on; `model delete` and `--partial` remove a split download's parts, resume files, and interrupted join; `NewEngine` / `ApplyMTPDefaults` suppress external MTP for DeepSeek V4.1 as the CLI does.
 * **Security**: `visit_page` now applies `fetch_image`'s destination policy (http(s) only; no loopback, private, or link-local hosts unless `AllowPrivateFetch`), checked again on the page it lands on; `fetch_image` enforces that policy on the address it dials, closing a DNS-rebinding bypass; and the library search never falls back to a bare `libds4.*` name, which dyld and Windows resolve through the working directory.
 ...
 * **`model status`**: processes with an engine loaded and the models, MTP/DSpark companions, and vision encoders they hold open, mapped to catalog aliases and roles (from open files and run locks, nothing the engine writes); then every catalog model with partial data on disk, with state (downloading / stalled / interrupted, from the download lock and the part file's growth), the downloader's PID, progress, a sampled rate, and an ETA; `--watch` and `--json`. Downloads also resume after transient network errors with backoff, and the retry budget resets on progress.
 ...
 * **Fixes**: a failed multi-image `ChatAppendMultimodalMessage` left Go handles pointing at buffers libds4 had already replaced and freed (double free on cleanup); `/read` kept the image path instead of its bytes, so re-rendered history broke if the file moved; `ImageEncoder.SetLimits(0, ...)` still cached one entry; `Session.TokenLogprob` read `ds4_session_token_logprob`'s success return (1) as an error, so it failed on every real call; the example server rendered prompts in thinking mode but stop-detected as if thinking were off, so every completion came back empty. The example server also parsed every completion as a thinking, DSML-V4 reply, so with `reasoning_effort: "none"` the answer was filed as reasoning and returned empty, and a V4.1 engine's DSML41 tool calls were dropped; it now parses under the request's think mode and `ToolSyntax(engine)`. `BuildChatPrompt` and the tool loop asked the process-wide default library whether a think mode replays reasoning, so an explicitly loaded library with no default silently dropped assistant reasoning from replayed turns; `Library.ThinkModeEnabled` and `Engine.ThinkModeEnabled` bind `ds4_think_mode_enabled` on the library in hand (V4.1 level 0 counts as off) and the prompt builders use the engine's. `model delete` and `model delete --partial` removed only `<model>.part`, so a split download's published parts, their resume files, and an interrupted `.assembling` join survived (and `--partial` failed outright); both now remove every piece under the same download lock, and `delete` still clears the default when the model was it. Root `NewEngine` and `ApplyMTPDefaults` suppressed an external MTP path only for GLM; they now do so for catalog DeepSeek V4.1 models too, including through the active-model hard link, matching the CLI; explicit `ds4api.EngineOptions` are untouched.
 ...
 * **Security**: `webtool.VisitPage` refused `file://`, `chrome://`, and other non-http(s) URLs and, unless `AllowPrivateFetch`, loopback, private, and link-local destinations, including the page a redirect lands on; a model steered by page content could otherwise read local files or reach internal services through the browser. `fetch_image` resolves the host itself and dials the vetted address, so a DNS answer that changes between the check and the connect no longer bypasses the policy; a custom `HTTPClient` must use an `*http.Transport` unless `AllowPrivateFetch` is set. `DefaultLibraryPath` and `ds4api.Load("")` return "not found" instead of the bare platform library name, and `ds4api.Load` refuses a bare name: dyld and Windows `LoadLibrary` search the working directory for a leaf name, so an uninstalled ds4go run from a directory holding a planted `libds4` would have loaded it with every integrity check skipped.
